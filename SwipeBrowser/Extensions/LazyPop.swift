...
extension SlideAnimatedTransitioning: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition (using transitionContext: UIViewControllerContextTransitioning) {
        ...
        var offsetLeft: CGRect! = fromView?.frame
        offsetLeft?.origin.x = width

        var offscreenRight: CGRect! = toView?.frame
        offscreenRight?.origin.x = -width / CGFloat(3.33)

        toView?.frame = offscreenRight!

        fromView?.layer.shadowRadius = CGFloat(5.0)
        fromView?.layer.shadowOpacity = Float(1.0)
        toView?.layer.opacity = Float(0.9)
        ...
        UIView.animate (withDuration: transitionDuration (using: transitionContext), delay: 0, options: UIView.AnimationOptions.curveLinear, animations: {
            ...
            toView?.layer.opacity = Float(1.0)
            fromView?.layer.shadowOpacity = Float(0.1)
        }, completion: { (finished: Bool) in
            toView?.layer.opacity = Float(1.0)
            toView?.layer.shadowOpacity = Float(0.0)
            fromView?.layer.opacity = Float(1.0)
            fromView?.layer.shadowOpacity = Float(0.0)
